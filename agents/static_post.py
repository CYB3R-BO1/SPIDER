...
class StaticPost:
    ...
    def remove_unreachable_blocks(graph_store, func_addr: int) -> int:
        ...
        for bb in unreachable:
            # Remove edges from/to unreachable block
            for s, d in edges:
                if s == bb or d == bb:
    ...
    def fold_linear_blocks(graph_store, func_addr: int) -> int:
        ...
        for bb in blocks:
            if bb in visited:
                continue
            # Start of a chain: bb has exactly one successor whose only predecessor is bb
            chain = [bb]
            current = bb
            while True:
                s = succs.get(current, [])
    ...
    def detect_switch_tables(graph_store, func_addr: int) -> List[Tuple[int, List[int]]]:
        ...
        for bb in blocks:
            ...
            if not ops:
                continue
            # Look for indirect table pattern: [reg + reg*scale]
    ...
    def infer_prologue_epilogue(graph_store, func_addr: int) -> Dict:
        ...
        # Check epilogue in last block (block with ret)
        for bb in blocks:
            insns = graph_store.fetch_block_instructions(bb)
            if not insns:
                continue
